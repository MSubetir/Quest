...
function abstract(i){
    ...
    if(curMission[i].abstract.substr(1, 3) == " - "){
        ...
        diffDays -= 1;
        console.log(now.getTime(), data.getTime(), timeDiff, diffDays)
        if(curMission[i].abstract.substr(0, 1) == "C"){
            ...
            parseInt(resto) - diffDays >= 0 ? curMission[i].abstract = parseInt(resto) - diffDays : curMission[i].abstract = 0;
            curMission[i].title = "- "+curMission[i].title+" -";
            ...
            if(editState == true){
                ...
                if(cancel != true){
                    ...
                    if(quest[i].abstract.substr(0, 4) == "C - " || quest[i].abstract.substr(0, 4) == "R - "){
                        ...
                        if(quest[i].abstract.substr(quest[i].abstract.length - 2, 1) == "/"){
                            info = quest[i].abstract.substr(0, quest[i].abstract.length - 1)
                            abstract = quest[i].abstract.substr(quest[i].abstract.length - 1, 1);
                            console.log(info+abstract)
                        }else if(quest[i].abstract.substr(quest[i].abstract.length - 3, 1) == "/"){
                            info = quest[i].abstract.substr(0, quest[i].abstract.length - 2)
                            abstract = quest[i].abstract.substr(quest[i].abstract.length - 2, 2);
                            console.log(info+abstract)
                        }else if(quest[i].abstract.substr(quest[i].abstract.length - 4, 1) == "/"){
                            info = quest[i].abstract.substr(0, quest[i].abstract.length - 3)
                            abstract = quest[i].abstract.substr(quest[i].abstract.length - 3, 3);
                            console.log(info+abstract)
                        }else if(quest[i].abstract.substr(quest[i].abstract.length - 5, 1) == "/"){
                            info = quest[i].abstract.substr(0, quest[i].abstract.length - 4)
                            abstract = quest[i].abstract.substr(quest[i].abstract.length - 4, 4);
                            console.log(info+abstract)
                        }else if(quest[i].abstract.substr(quest[i].abstract.length - 6, 1) == "/"){
                            info = quest[i].abstract.substr(0, quest[i].abstract.length - 5)
                            abstract = quest[i].abstract.substr(quest[i].abstract.length - 5, 5);
                            console.log(info+abstract)
                        }else if(quest[i].abstract.substr(quest[i].abstract.length - 7, 1) == "/"){
                            info = quest[i].abstract.substr(0, quest[i].abstract.length - 6)
                            abstract = quest[i].abstract.substr(quest[i].abstract.length - 6, 6);
                            console.log(info+abstract)
                        }else if(quest[i].abstract.substr(quest[i].abstract.length - 8, 1) == "/"){
                            info = quest[i].abstract.substr(0, quest[i].abstract.length - 7)
                            abstract = quest[i].abstract.substr(quest[i].abstract.length - 7, 7);
                            console.log(info+abstract)
                        }
                        
                        quest[i].abstract = prompt("Número natural", abstract);
                        ...
                        if(cancel == false && parse != check || cancel == false && parse < 0){
                            ...
                            let checkq;
                            do {
                                quest[i].abstract = prompt("Número natural necessário", abstract);
                        ...
                        let novo = new Date;
                        quest[i].abstract = info.substr(0, 4) +novo.getDate()+"/"+novo.getMonth()+"/"+novo.getFullYear()+"/"+quest[i].abstract;
                        
                        if(cancel == true){
                            quest[i].abstract=info+(backup_abstract.substr(backup_abstract.length - 1, 1));
                            quest[i].title = backup_title;
                            numEditado = 1000;
                            numBackup = "";
                        }
                        ...
                        console.log(backup_abstract, quest[i].abstract, info)
                        editState = false;
                        resumo.disabled = true;
                    }else{
                        numEditado = i;
